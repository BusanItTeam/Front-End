...
const OrderComplete = () => {
  ...
  return (
    <div className="p-10 max-w-5xl mx-auto mt-50 mb-60 bg-gray-100 rounded-lg">
      <h1 className="text-3xl font-bold mb-6 text-center text-gray-900">🎉 주문이 완료되었습니다!</h1>

      {/* <div className="bg-white p-6 rounded-lg shadow-md">
        <h2 className="text-xl text-gray-800 mb-4 font-bold border-b pb-2">주문 상세</h2>
            ...
            <tbody>
              {cartItems.map((item) => (
                <tr key={item.id} className="border-b hover:bg-gray-100">
                  ...
                  <td className="p-4 text-yellow-600 font-semibold">🚚 배송 준비 중</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="mt-10 flex justify-center">
        <Link to="/" className="w-1/2 mx-5 bg-gray-900 text-white py-3 rounded-lg font-medium hover:bg-gray-800 text-center text-lg">
          홈으로 이동
        </Link>
        <Link to="/orders/history" className="w-1/2 mx-5 bg-gray-900 text-white py-3 rounded-lg font-medium hover:bg-gray-800 text-center text-lg">
          주문 상세보기
        </Link>
      </div>
    </div>
