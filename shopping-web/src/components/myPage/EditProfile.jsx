...
const EditProfile = () => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const response = await api.put("/auths/user", {
        name: name || null,
  ...
  return (
    ...
    <h2 className="text-xl font-semibold text-red-500 mb-4">  {name} 님의 Edit Your Profile</h2>
    ...
    <div className="col-span-2">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700">아파트명 및 명칭</label>
        <input
          type="text"
          placeholder="대동아파트"
      ...
      <div className="col-span-2">
        <label className="block text-sm font-medium text-gray-700">계정 생성일</label>
        <input  
          type="text"
          placeholder="2024-02-01"
          defaultValue={currentUser?.createdDate ? formatDate(currentUser.createdDate) : ""}
          className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500"
        />
      </div>
      
      <div className="col-span-2 flex justify-between mt-4">
        <button className="text-gray-500 hover:underline" onClick={handleGoToMyPage}>Cancel</button>
        <button className="bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-600" onClick={handleUpdate}>Save Changes</button>
      </div>
    </div>
  </div>
  );
